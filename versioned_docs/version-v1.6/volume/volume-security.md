...
- Secret: A Kubernetes secret is used as the passphrase of dm_crypt. You must specify the value of the `CRYPTO_KEY_VALUE` field. All other fields are fixed.
  ...
  ![](/img/v1.4/image-hv/create-encryption-used-secret.png)
...
- StorageClass: Images are encrypted using Longhorn, so required fields must be passed to the Longhorn CSI Driver. You can specify the encryption secret when creating a StorageClass. For more information, see [Image StorageClass](../image/upload-image#image-storageclass).
  ...
  ![](/img/v1.4/image-hv/create-storage-class.png)
...
- **Volumes: Create** screen
  ...
  ![create-empty-volume](/img/v1.4/volume-hv/create-empty-volume.png)

- **Volumes** tab on the **Virtual Machine: Create** screen

  ![create-empty-volume](/img/v1.4/volume-hv/create-empty-volume-in-vm.png)
