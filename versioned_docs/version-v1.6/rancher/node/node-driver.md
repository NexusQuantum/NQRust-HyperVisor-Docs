---
sidebar_position: 1
sidebar_label: Hypervisor Node Driver
title: "Hypervisor Node Driver"
keywords:
  - Hypervisor
  - hypervisor
  - Rancher
...
description: The Hypervisor node driver is used to provision VMs in the Hypervisor cluster. In this section, you'll learn how to configure Rancher to use the Hypervisor node driver to launch and manage Kubernetes clusters.
---
...
The [Hypervisor node driver](https://github.com/harvester/docker-machine-driver-harvester), similar to the Docker Machine driver, is used to provision VMs in the Hypervisor cluster, and Rancher uses it to launch and manage Kubernetes clusters.
...
Additionally, the Hypervisor node driver is integrated with the Hypervisor cloud provider by default, providing [built-in load balancer support](../cloud-provider.md#load-balancer-support) as well as [storage passthrough](../csi-driver.md) from the bare-metal cluster to the guest Kubernetes clusters to gain native storage performance.

In this section, you'll learn how to configure Rancher to use the Hypervisor node driver to launch and manage Kubernetes clusters.

:::note

The Hypervisor node driver only supports cloud images. This is because ISO images usually require additional setup that interferes with a clean deployment (without requiring user intervention), and they are not typically used in cloud environments.

:::

## Hypervisor node driver

Starting from Rancher `v2.6.3`, the Hypervisor node driver is enabled by default. You can go to the **Cluster Management** > **Drivers** > **Node Drivers** page to check the Hypervisor node driver status.

![edit-node-driver](/img/v1.2/rancher/edit-node-driver.png)

When the Hypervisor node driver is enabled, you can create Kubernetes clusters on top of the Hypervisor cluster and manage them from Rancher.

![hypervisor-node-driver](/img/v1.2/rancher/harvester-node-driver.png)
...
provision guest Kubernetes clusters on Hypervisor. When these tokens expire,
...
Kubernetes clusters running on Hypervisor and using cloud credentials created
...
Furthermore, for custom installation, the Hypervisor cloud provider version should be `>= v0.1.4`.

### Sync topology labels to the guest cluster node

During the cluster installation, the Hypervisor node driver will automatically help synchronize topology labels from VM nodes to guest cluster nodes. Currently, only `region` and `zone` topology labels are supported.

1. Configure topology labels on the Hypervisor nodes on the **Hosts** > **Edit Config** > **Labels** page. For example, add the topology labels as follows:
   ```yaml
...
1. Create a downstream RKE2 cluster using the Hypervisor node driver with Hypervisor cloud provider enabled. We recommend adding the [node affinity rules](./rke2-cluster.md#add-node-affinity), which prevents nodes from drifting to other zones after VM rebuilding.
